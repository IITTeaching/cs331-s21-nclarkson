...
def check_delimiters(expr):
    ...
    for c in expr:
        if delim_openers.__contains__(c):
            ...
            if (s.peek() == None):
                return False
            pos = delim_openers.index(s.peek())
            if (delim_closers[pos] == c):
                s.pop()
            else:
                return False
    return s.empty()
    ...
    for t in toks:
        if t.isdigit():
            ...
            while done == False:
                ...
                elif prec.get(t) > prec.get(ops.peek()):
                    ops.push(t)
                    done = True
                elif prec.get(t) == prec.get(ops.peek()):
...
class Queue:
    ...
    def enqueue(self, val):
        ### BEGIN SOLUTION
        if (self.data[0] == None) & (self.empty() == False):
            pos = -1
            self.tail = -1
            for i in range(len(self.data)):
                if self.data[i] != None:
                    ...
                    if pos == -1:
                        pos = i
                    self.data[i - pos] = self.data[i]
                    self.data[i] = None
        if self.data[len(self.data) - 1] != None:
            raise RuntimeError
        self.tail += 1
